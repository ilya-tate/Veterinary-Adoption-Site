...
const getAllAnimals = async (req, res) => {
  ...
  try {
    if (pageNumber === 1) {
      animals = await AnimalModel.find().limit(size).sort({ createdAt: -1 });
    } else {
...
const getDisplayAnimals = async (req, res) => {
  ...
  try {
    let animals = await AnimalModel.find().limit(size).sort({ createdAt: -1 });
...
module.exports = { getDisplayAnimals, getAllAnimals, oneAni, getAnimalById };
